...
public class CustosApplication extends BasicLoggedApplication {
    ...
    public static void main(final String... args) {
        ...
        try {
            ...
            final CustosApplication application = new CustosApplication(
                    applicationProperties);
            application.start();
        } catch (final Exception e) {
            getLogger().fatal(
                    "Error while starting application. Shutting down.", e);
        }
    }

    public CustosApplication(final ApplicationProperties applicationProperties) {
        super();
        this.session = new BasicSession(applicationProperties,
                new SimpleEventBus(), getLogger());
        ...
        this.session.getEventBus().register(this.systemMessageMemory,
                SystemMessage.class);
        this.gui = new CustosGui(this.session, this.colorProvider,
                this.systemMessageMemory);
    ...
    @Override
    public void start() {
        this.session.getEventBus().publish(
                new SystemMessage(MessageSeverity.SUCCESS, new DirectI18N(
                        "Starting application").toString()));
        super.start();
        for (final CustosModule custosModule : this.modules) {
            this.session.getEventBus().register(custosModule,
        ...
        this.session.getEventBus().publish(
                new SystemMessage(MessageSeverity.SUCCESS, new DirectI18N(
                        "Custos ${0} successfully started.", this.session
                                .getProperties().version()).toString()));
    }
}
